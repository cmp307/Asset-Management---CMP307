...
def gif():
    ...
    loadWindow = pyGUI.Window('loading').Layout(layout)
    ...
    while True:
        ...
        if (loaded):
            ...
            break
        if not (loaded):
            loadWindow.Element('_IMAGE_').UpdateAnimation(gif103,  time_between_frames=50)
...
def main():
    ...
    while True:
        ...
        if event == 'Check table for vunerabilities':
            ...
            a = threading.Thread(target=test, daemon=True).start()
            b = gif()
            window = reloadFrame(window, b)
